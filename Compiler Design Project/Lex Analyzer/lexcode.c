...
int token_ptr = 0;		// increment at each insertion for new token in token_list
...
int tokenizefull(){
	...
	while(gets(a)){
		current_line_no++;
		char *tok= strtok(a," \t\n"); //STRTOK = DIVIDE COMPONENTS WHEN THERE IS SPACE IN INPUT
...
//YYLEX() RETURNS A VALUE INDICATING A TYPE OF TOKEN THAT HAS BEEN OBTAINED.
int yylex(){
	...
	current_token_ptr++; //incrementing
...
//COMPARES THE INPUT VALUE GIVEN AND DOES OPERATION	
	if ( strcmp(temp, ";")== 0 ) { //STRCMP= STRING COMPARE
		token_list[token_ptr].token_class = semi_colon; //assigning class name
		token_list[token_ptr].token_text = ";"; //value assign
		token_list[token_ptr].line_no = current_line_no; //line number
		token_ptr++;
		return true;
		}
		...
		if(check_regex(temp)){			//if regex then return identifier else error
			return true;
		}
		else 
			return false;	
...
int check_regex(char *temp){
	...
	reti1 = regcomp(&regex1, pattern1, 0); //compiling regular expression
...
char* substring(char* input, int start, int end){ 
	...
	//fucntion is used to copy a block of data from one location to another
	//temp is destination,
	temp[end-start] = '\0';
	return temp;
}
...
char* yytext(){ //returns text inside the given input

	return token_list[current_token_ptr].token_text;

} 

int yylineno(){  return token_list[current_token_ptr].line_no;} //returns token number
...
int main(void)
{
	...
	ntoken = yylex();
	while(ntoken){
		printf("%d:%s:%d\n", ntoken,yytext(),yylineno()); //printing tokennum:text_in_input:lineno
		ntoken=yylex();
	}
	return 0;
}
